        ...
        interface validate_person_interface
        {
            ...
            public  function validate_data(array $data_array);
        }
        ...
        class myException extends Exception
        {
            ...
            {
                $all_errors = $this->getTrace();
                $err_string = '';

                foreach ($all_errors as $error) {
                    $err_string .= $error['function'] . ': ' . '<em>Error [' . $this->getCode() . "] </em><br>" . $this->message . 
        ...
        class Person implements validate_person_interface
        {
            ...
            public final function add_attribute(string $name, string $type, array $properties = [])
            {
                ...
                if (count($properties) > 0) {
                    ...
                    foreach ($properties as $property_name => $property_value) {
                        ...
                        if (!in_array($property_name, self::ALLOWED_PROPERTIES)) {
                            throw new myException("'$property_name' is not a valid property!", 2);
                            return false;
                        }
            ...
            public final function remove_attribute(string $name)
            {
                unset($this->custom_rules[$name]);
            }

            public final function validate_data(array $data_array)
            {
